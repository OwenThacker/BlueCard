...
def process_login(n_clicks, email, password):
    ...
    session_data = {}
    success_alert = None
    error_alert = dbc.Alert("Invalid email or password", color="danger")
    ...
    if user_data and check_password_hash(user_data[2], password):
        ...
        print("Session Data after login:", session_data)  # Debugging line
        return success_alert, session_data, "/dashboard"  # Redirect to a dashboard or home page
...
# Updated callback to check session data on app start
@app.callback(
    Output('auth-overlay', 'style', allow_duplicate=True),
    Input('session-data-store', 'data'),
    prevent_initial_call=True
)
def check_user_session(session_data):
    print("Session Data Received in Overlay Check:", session_data)  # Debugging line
    if session_data and 'user_id' in session_data:
        return {'display': 'none'}  # Hide the overlay if the user is logged in
    else:
        return {'display': 'flex'}  # Show the overlay if the user is not logged in
...
def update_login_overlay(session_data, pathname):
    ...
    # Handling logic for the session data or pathname
    if pathname == '/':
        return {'display': 'none'}  # No overlay on the home page

    if not session_data or 'user_id' not in session_data:
        return {
            'display': 'block',  # Show the overlay if not authenticated
